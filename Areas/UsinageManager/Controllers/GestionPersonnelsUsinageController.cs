...
namespace DevKbfSteel.Controllers
{
    ...
    public class GestionPersonnelsUsinageController : Controller
    {
        private KBFsteelContext _context;
        public GestionPersonnelsUsinageController(KBFsteelContext context) {
            _context = context;
        }
        [HttpGet]
        public async Task<IActionResult> Get(DataSourceLoadOptions loadOptions) {
            var intervenants = _context.RhListeDesEmployes
                ...
                .Where(c =>c.Departement == XpertHelper.CodeUsinage)
                .Select(i => new {
                    i.Id,
                    i.Nom,
                    i.Prenom,
        ...
        public async Task<IActionResult> NomLookup(DataSourceLoadOptions loadOptions)
        {
            var lookup = from i in _context.RhListeDesEmployes
                         orderby i.Id
                         where i.Departement == XpertHelper.CodeUsinage && i.Disponnible != 3 && i.Disponnible != null
                         select new
                         {
                             Value = i.Id,
