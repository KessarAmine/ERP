...
namespace DevKbfSteel.Areas.MagasinSuperviseur.Controllers
{
    ...
    public class ReintegrationController : Controller
    {
        ...
        {
            XpertHelper.NumReintegration = id;
        ...
        public async Task<IActionResult> PostReintegrationDetails(string values)
        {
            ...
            var StkReintegration = _context.StkReintegration
                .AsNoTracking()
                .Where(o => o.NumBonReintegration == XpertHelper.NumReintegration)
                .Select(i => new
                {
                    i.DateReingegration
                }).ToList();
            ...
            if (StkReintegration.Count() > 0)
            {
                ...
                model.NumBonReintegration = XpertHelper.NumReintegration;
        ...
        //=================================================Deletes===================================
        [HttpDelete]
        public async Task<IActionResult> DeleteReintegration(int key)
        {
            var model = await _context.StkReintegration.FirstOrDefaultAsync(item => item.NumBonReintegration == key);
            var details = _context.StkReintegrationArticles.Where(item => item.NumBonReintegration == key).ToList();
            if (details.Count > 0)
