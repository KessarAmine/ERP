...
namespace DevKbfSteel.Areas.MagasinSuperviseur.Controllers
{
    ...
    public class FraisApprochesController : Controller
    {
        ...
        public async Task<IActionResult> PostFraisEntree(string values)
        {
            ...
            //update all entrees avec new frais 
            ...
            if (entreesArticles.Count() > 0)
            {
                ...
                //sum of all entree articles
                foreach (var itemStkBonEntreeArticles in entreesArticles)
                {
                    sommeEntree += Math.Round((double)(itemStkBonEntreeArticles.QteRecu * itemStkBonEntreeArticles.PrixUnitaire), 4); ;
                }
                foreach (var itementreesArticles in entreesArticles)
                {
                    //if this article is part of frais global
                    // we dont touch article that have their own cout
                    if (String.IsNullOrEmpty(itementreesArticles.ValeurFrais.ToString()))
                    {
                        // calculate cout
                        itementreesArticles.CoutUnitaire = Math.Round((itementreesArticles.PrixUnitaire * SommeFraisGeneral * itementreesArticles.QteRecu) / sommeEntree, 4);
                        itementreesArticles.Montant = Math.Round((double)(itementreesArticles.QteRecu * itementreesArticles.PrixUnitaire)+ SommeFraisGeneral, 4);
                        _context.StkBonEntreeArticles.Update(itementreesArticles);
