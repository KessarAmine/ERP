...
namespace DevKbfSteel.Areas.MagasinSuperviseur.Controllers
{
    ...
    public class SortiesController : Controller
    {
        ...
        public void updatehoursortie()
        {
            var sorties = _context.StkBonSortie.ToList();
            foreach (var sortie in sorties)
            {
                sortie.DateSortie = sortie.DateSortie.AddHours(-10.0);
                _context.StkBonSortie.Update(sortie);
                _context.SaveChanges();
                var details = _context.StkBonSortieArticles.Where(c => c.NumBonSortie == sortie.NumBonSortie);
                foreach (var detail in details)
                {
                    var movement = _context.StkMovements.Where(c => c.TypeMovement == XpertHelperGestionnaireMagasin.typeMovment_Sortie 
                                                                && c.IdDetail == detail.Id).FirstOrDefault();
                    movement.DateMovment = sortie.DateSortie;
        ...
        {
            XpertHelperMagasinSuperviseur.NumBonSortieMagasin = id;
        ...
        public async Task<IActionResult> Post(string values)
        {
            ...
            model.CodeServiceEmetteur = XpertHelperMagasinSuperviseur.CodeMagasin;
            model.NumDemandeFourniture = XpertHelperMagasinSuperviseur.NumDemandeFournitureMagasin;
        ...
        public async Task<IActionResult> PostDetail(string values)
        {
            ...
            model.NumBonSortie = XpertHelperMagasinSuperviseur.NumBonSortieMagasin;
        ...
        {
            var model = await _context.StkBonSortie.FirstOrDefaultAsync(item => item.NumBonSortie == key);
            var details = _context.StkBonSortieArticles.Where(item => item.NumBonSortie == key).ToList();
            if (details.Count > 0)
